...
@section('title',  __('adminlte::adminlte.managementBoard') . '-' . __('adminlte::adminlte.editNews') )

@section('content_header')
    <div class="row">
        <ol class="breadcrumb float-sm-left">
            <li class="breadcrumb-item"><a
                    href="{{ localizedRoute('management-board.index') }}">{{ __('adminlte::adminlte.managementBoard') }}</a></li>
...
@section('content')
    ...
    @endif
    <form action="{{ localizedRoute('management-board.update', ['management_board'=>$post->id]) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
            ...
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-12 col-md-6"> 
                        <div class="form-group">
                            <x-adminlte-input  
                                    name="{{ $post->order!=0? 'content_ar': 'slug' }}"  
                                    value="{{ $post->order!=0? $post->postDetailOne->content : $post->postDetailOne->slug}}"
                                label="{{ __('adminlte::adminlte.jobAR') }}"
                        ...
                        <div class="form-group">
                            <x-adminlte-input 
                                name="{{ $post->order!=0? 'content_en': 'slug_en' }}"  
                                value="{{ $post->order!=0? $post->postDetailOne->content_en : $post->postDetailOne->slug_en}}"
                                label="{{ __('adminlte::adminlte.jobEN') }}"
                                placeholder="{{ __('adminlte::adminlte.job') }} .." label-class="text-olive" enable-old-support>
                                <x-slot name="prependSlot">
                                    <div class="input-group-text">
                                        <i class="fas fa-user-tie"></i>
