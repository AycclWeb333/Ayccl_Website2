...
@section('content')
    ...
    <div class="container mx-0  mb-5">
        <a href="{{ route('hadhrami.create', ['locale'=> app()->getLocale()]) }}">
    ...
    @php
        $heads = [
            ...
            // ['label' => __('adminlte::adminlte.category'), 'classes' => 'border border-white bg-olive'],
            [
        ...
        foreach ($posts as $post) {
            ...
            $btnEdit =
                '<a href="' .
                route('hadhrami.edit', ['hadhrami' => $post->id, 'locale' => app()->getLocale()]) .
        ...
        $config = [
            ...
            // 'serverSide' => true,
            // 'processing' => true,
            // 'ajax' => "{{ localizedRoute('hadhrami.index') }}",
            'language' => [
    ...
    <script>
        $(document).ready(function() {
            $('#modalCustom').on('show.bs.modal', function(event) {
                ...
                var deleteAction =
                    '{{ route('hadhrami.destroy', ['locale' => app()->getLocale(), 'hadhrami' => '__ID__']) }}';
            ...
            $('#modalActivate').on('show.bs.modal', function(event) {
                ...
                // Assuming 'hadhrami.activate' is a route for activating hadhrami. Adjust as needed.
                var action =
