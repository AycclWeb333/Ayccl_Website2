...
class NewsController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('admin-panel.media-center.news.create', compact('pages', 'categories'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $post->page_id = 51; // default page
            $post->date = $request->date;
            if (isset($request->order))
                $post->order     = $request->order;
            else {
                $maxOrder = Post::where('page_id', 51)->where('active', true)->max('order');
            ...
            if ($request->hasFile('files')) {
                ...
                foreach ($files as $file) {
                    ...
                    $originalRel   = "images/news/{$post->id}/{$uniqueName}";
                    $thumbRel      = "images/news/{$post->id}/thumbnails/{$uniqueName}";

                    $originalDir   = public_path("images/news/{$post->id}");
                    $thumbDir      = public_path("images/news/{$post->id}/thumbnails");
                ...
                DB::commit();
            return redirect()->route('news.index', app()->getLocale())
    ...
    public function edit($locale, int $id)
    {
        ...
        return view('admin-panel.media-center.news.edit', compact('post'));
    }
    ...
    public function destroy($locale, int $id)
    {
        try {
            ...
            return redirect()->route('news.index', app()->getLocale())->with(['success' => __('adminlte::adminlte.succDelete')]);
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->back()->with(['error' => $e->getMessage()]);
        }
